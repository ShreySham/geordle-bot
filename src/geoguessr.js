...
async function getChallengeLink(optionalMap) {
  ...
  try {
    ...
    console.log("gotHome");
    await Promise.all([
      tab.waitForSelector(".quick-search_searchInput__i7C6O", {
        timeout: 7000,
      }),
      tab.click(".quick-search_searchInputButton__kK9Hz"),
    ]);
    await tab.type(".quick-search_searchInput__i7C6O", mapName);
    await tab.waitForSelector(".quick-search_resultList__cLjml", {
      timeout: 7000,
    });
    ...
    await Promise.all([
      tab.waitForNavigation({ timeout: 7000 }),
      tab.click(
        'li[class="search-result_wrapper__jwHxo search-result_isFocused__ZAXVX"]'
      ),
    ]);
    ...
    await tab.evaluate(() => {
      document
        .querySelector(
          "#__next > div.background_wrapper__OlrxG.background_backgroundClassic__ySr_Z > div.version4_layout__KcIcs > div.version4_content__oaYfe > main > div > div > div > div > div.section_sectionMedium__yXgE6 > div > div > div:nth-child(2) > input"
        )
        .click();
    });
    ...
    await tab.evaluate(() => {
      document
        .querySelector(
          "#__next > div.background_wrapper__OlrxG.background_backgroundClassic__ySr_Z > div.version4_layout__KcIcs > div.version4_content__oaYfe > main > div > div > div > div > div:nth-child(2) > div:nth-child(2) > label > input[type=radio]"
        )
        .click();
    });
    ...
    await Promise.all([
      tab.waitForSelector(
        "#__next > div.background_wrapper__OlrxG.background_backgroundClassic__ySr_Z > div.version4_layout__KcIcs > div.version4_content__oaYfe > main > div > div > div > div > section > article > div > span > input",
        { timeout: 7000 }
      ),
      tab.evaluate(() => {
        document
          .querySelector(
            "#__next > div.background_wrapper__OlrxG.background_backgroundClassic__ySr_Z > div.version4_layout__KcIcs > div.version4_content__oaYfe > main > div > div > div > div > div.start-challenge-game_body__sWqnL > button > div"
          )
          .click();
      }),
    ]);

    const linkBox = await tab.waitForSelector(
      "#__next > div.background_wrapper__OlrxG.background_backgroundClassic__ySr_Z > div.version4_layout__KcIcs > div.version4_content__oaYfe > main > div > div > div > div > section > article > div > span > input"
    );
